...
beegfs_ha_fencing_agents:
  fence_redfish:
    ...
    - pcmk_host_list: <HOSTNAME>
      ...
      <<: *redfish
----
... Per ulteriori informazioni sulla configurazione di altri agenti di scherma, fare riferimento alla link:https://docs.redhat.com/en/documentation/red_hat_enterprise_linux/9/html/configuring_and_managing_high_availability_clusters/assembly_configuring-fencing-configuring-and-managing-high-availability-clusters["Documentazione di Red Hat"^].
...
CAUTION: Per evitare conflitti quando le interfacce vengono rinominate (impedendone la ridenominazione), non utilizzare nomi predefiniti potenziali come eth0, ens9f0, ib0 o ibs4f0. Una convenzione di denominazione comune prevede l'utilizzo di 'e' o 'i' per Ethernet o InfiniBand, seguito dal numero dello slot PCIe e da una lettera che indica la porta. Ad esempio, la seconda porta di un adattatore InfiniBand installato nello slot 3 è: I3b.

+

NOTE: Se si utilizza un modello di nodo file verificato, fare clic su link:https://docs.netapp.com/us-en/beegfs/second-gen/beegfs-deploy-create-inventory.html#step-4-define-configuration-that-should-apply-to-all-file-nodes["qui"^] per visualizzare, ad esempio, le mappature indirizzo PCIe-porta logica.
...
Fai clic link:https://github.com/NetApp/beegfs/blob/master/getting_started/beegfs_on_netapp/gen2/H2_DAC_IB_A2_EF600/beegfs_cluster_building_blocks/beegfs_ha_building_block_cluster_1/group_vars/ha_cluster.yml["qui"^] per un esempio di file di inventario completo che rappresenta una configurazione comune dei nodi file.
