...
+
Le impostazioni di sistema variano a seconda del modello di server utilizzato come nodo di file. Le impostazioni devono essere configurate manualmente in base al modello di server in uso. Per informazioni su come configurare le impostazioni di sistema per i nodi di file Lenovo SR665 V3 convalidati, vedere link:beegfs-deploy-file-node-tuning.html["Ottimizzare le impostazioni del sistema del nodo di file per le performance"] .
